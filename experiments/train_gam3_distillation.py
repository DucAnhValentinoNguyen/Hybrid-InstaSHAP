...
# --- 4. STUDENT GAM-3 ARCHITECTURE (SURGICAL) ---
print(">>> Initializing Student GAM-3...")
class InstaSHAP_GAM3(nn.Module):
    def __init__(self, embed_dim=128):
        ...
        # SURGICAL LOCK: Freeze layers 0-6
        ...
        self.head_order3 = nn.Linear(embed_dim, NUM_CLASSES) # The Triplet Head
    ...
    def forward(self, x, mask):
        ...
        # Order 3 (Triplets)
        term1 = sum_h ** 3
        term2 = 3 * (sum_h * sum_h_sq)
        term3 = 2 * sum_h_cubed
        o3 = self.head_order3((1/6.0) * (term1 - term2 + term3))
        
        return o1 + o2 + o3

student = InstaSHAP_GAM3().to(device)
...
optimizer = optim.Adam([
    ...
    {'params': student.head_order3.parameters(), 'lr': 1e-3}
])

# Define the missing criterion
criterion_ce = nn.CrossEntropyLoss(label_smoothing=0.1)

# Define the missing scheduler
scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=2)

# Define the missing scaler for mixed precision
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_acc:
            best_acc = val_acc
            # Change filename to best_gam3_distilled.pth
            torch.save(student.state_dict(), "final_paper_results/best_gam3_distilled.pth")
